...
1. Pull BTC and ETH spot (Coinbase first on the Pi; Binance often 451s in the US).
2. Estimate hourly vol from recent 1-minute candles (fallback: BTC 0.4%/hour, ETH 0.5%/hour).
...
8. `scan` / `once` only print. `live` posts a post-only GTC limit. Typical ticket about $2, never over $3. If that price would take the book, it steps one tick more passive. It will not market-buy.

## Rules it will not break

- Bankroll used for sizing: $40 unless you set `BANKROLL`.
- Preferred risk $2, hard cap $3, 5% of bankroll, quarter-Kelly.
- One idea per run.
- Ask between 5¢ and 95¢.
- Need 6% net edge, or 4% only if the book is tight (≤2¢) and deep (≥5 contracts).
- |z| > 2.5 needs 8% edge. |z| > 3.5 skip (news-only jump).
- Sit out CPI 8:15–8:45 AM ET and FOMC 1:45–2:45 PM ET on those days.
...
Same caps: one idea, about $2, $3 max, post-only. If the Pi was off at :03, it does not fire late.
...
- Spot / vol — Coinbase proxy, not the settlement index.
...
- Binance 451 — US geo-block. Coinbase is the fallback. Set `SPOT_SOURCE=coinbase`.
