"""Matt's 15m edge loop: first 2–4 minutes, Pass/Fail vs mid, one idea.

This is not a scan-all-day scalp. Maker (last 3 min 74–93¢) and hourly stay separate.
"""
...
def half_sigma_move(spot_now: float, spot_then: float, hour_vol: float, threshold: float = HALF_SIGMA) -> bool:
...
def news_blackout(now: datetime | None = None) -> str | None:
    ...
    if key in CPI_DATES:
        start = local.replace(hour=8, minute=30, second=0, microsecond=0) - timedelta(minutes=CPI_WINDOW_MINUTES)
        end = local.replace(hour=8, minute=30, second=0, microsecond=0) + timedelta(minutes=CPI_WINDOW_MINUTES)
        if start <= local <= end:
            return "CPI"
    if key in FOMC_DATES:
        start = local.replace(hour=14, minute=0, second=0, microsecond=0) - timedelta(minutes=FOMC_WINDOW_MINUTES)
        end = local.replace(hour=14, minute=0, second=0, microsecond=0) + timedelta(minutes=FOMC_WINDOW_MINUTES)
        if start <= local <= end:
            return "FOMC"
    return None
...
def pass_fail(
    *,
    ...
    """Grok Pass/Fail vs the live mid. Fail means skip — do not scalp it."""
